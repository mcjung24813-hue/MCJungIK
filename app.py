...
# --- 0. 페이지 설정 ---
st.set_page_config(layout="wide", page_title="Factory OS - Smart Input")

# 🔒 관리자 비밀번호 설정
ADMIN_PASSWORD = "1234"

MACHINE_DB_FILE = "factory_persistent_db.csv"
MASTER_DB_FILE = "factory_master_db.csv"

def load_machine_data():
    if os.path.exists(MACHINE_DB_FILE):
        try: return pd.read_csv(MACHINE_DB_FILE, index_col=0).to_dict('index')
        except: return {}
    return {}

def save_machine_data(data):
    pd.DataFrame.from_dict(data, orient='index').to_csv(MACHINE_DB_FILE)

def load_master_data():
    if os.path.exists(MASTER_DB_FILE):
        try: return pd.read_csv(MASTER_DB_FILE, index_col=0).to_dict('index')
        except: return {}
    return {}

def save_master_data(data):
    pd.DataFrame.from_dict(data, orient='index').to_csv(MASTER_DB_FILE)
...
# --- 1. CSS 디자인 (애플 스타일 화이트 테마 + 텍스트 크기 강조) ---
st.markdown("""
<style>
...
def get_random_color(): return "#{:06x}".format(random.randint(0, 0xFFFFFF))
...
def render_details_panel(m_name, m):
    ...
    st.markdown(f"""
<div class='modern-card' style='border-left: 5px solid #34c759;'>
<div class='card-title'>
🤖 {m_name} 상세 및 설정
<div class='status-container'>
<span class='status-dot {status_class}'></span>
<span class='status-text {text_class}'>{status_text}</span>
</div>
</div>
</div>
""", unsafe_allow_html=True)
    ...
    with c_btn1:
        if st.button("설정 적용", key=f"det_upd_p_{m_name}"):
            m['p_name'] = selected_p_name
            m['target'] = new_target
            m['count'] = new_count if new_count <= new_target else new_target
...
<div><b>{idx+1}.</b> <span class="schedule-date">{date_str}</span>{item['p_name']}</div>
<div>🎯 <b>{item['target']}</b> EA</div>
</div>
""", unsafe_allow_html=True)
            with del_col:
...
<div><b>{real_idx+1}.</b> <span class="schedule-date">{hist_date}</span>{item['p_name']}</div>
<div>✔️ <b>{item['count']}</b> / {item['target']} EA</div>
</div>
""", unsafe_allow_html=True)
            with readd_col:
        ...
        if st.button("🗑️ 내역 지우기", key=f"clear_hist_{m_name}"):
            m['history'] = []; save_machine_data(st.session_state.m_states); st.rerun()
...
def make_machine_card(m_name):
    ...
    if int(m.get('target', 0)) > 0 and int(m.get('count', 0)) >= int(m.get('target', 0)):
        m['count'] = int(m.get('target', 0))
        m['is_running'] = False  
        raw_memo = m.get('memo', '')
        if pd.notna(raw_memo) and str(raw_memo).strip() != "" and str(raw_memo).lower() != 'nan':
            m['memo'] = ""
    ...
    if cycle > 0 and count_val < target_val:
        ...
        h, rem = divmod(rem_sec, 3600)
        time_str = f"{int(h):02d}:{int(rem//60):02d}:{int(rem%60):02d}"
    elif count_val >= target_val and target_val > 0:
        time_str = "작업 완료"
    ...
    if m.get('schedule'):
        ...
        next_info = st.session_state.master_data.get(next_p, {})
        n_color = next_info.get("color_text", "미지정")
        next_color_html = f"<div class='next-color-tag'>⏭️ NEXT 컬러: {n_color}</div>"

    raw_memo = m.get('memo', '')
    memo_text = str(raw_memo).strip() if pd.notna(raw_memo) and str(raw_memo).lower() != 'nan' else ''
    memo_html = f"<div class='card-memo'>📌 {memo_text}</div>" if memo_text else ""

    # 제품코드의 크기도 약간 더 키워서 균형을 맞춤
...
{p_code_html} {safe_p_name} {color_html} {weight_html}
</div>
{next_color_html}
{memo_html}
...
"""
    ...
    if target_val > 0 and count_val >= target_val:
        ...
        if c1.button("▶️ START", key=f"run_{m_name}"):
            if m.get('p_name', '---') == '---':
                ...
                m['is_running'] = True; m['last_time'] = time.time()
    ...
    if c3.button("⚙️ Details", key=f"det_{m_name}"):
        st.session_state.selected_machine = m_name; st.rerun()
...
# 💡 [핵심 업데이트] 1층/3층 분리된 스마트 공정 계획표
with t_plan:
    st.subheader("📅 스마트 공정 계획표 (엑셀 뷰)")
    st.info("✨ **입력 방법:** 표 오른쪽의 **빈칸을 더블클릭**하고 **제품코드**를 입력 후 엔터를 치면 일정이 쏙 들어갑니다! (수량 지정: `코드/수량` 예: `1100265/500`)")
    
    with st.expander("➕ 새 공정 계획 수동 추가 (클릭하여 열기)", expanded=False):
        p_col1, p_col2, p_col3, p_col4 = st.columns(4)
        with p_col1:
            plan_m_name = st.selectbox("기계 선택", sorted(list(st.session_state.m_states.keys()), key=get_machine_sort_key))
        with p_col2:
            plan_p_name = st.selectbox("제품 선택", list(st.session_state.master_data.keys()))
        with p_col3:
            plan_target = st.number_input("목표 수량", min_value=1, value=1000, key="plan_qty")
        with p_col4:
            plan_date = st.text_input("일정/날짜", value=time.strftime("%Y-%m-%d"))
            
        if st.button("🚀 선택한 기계로 일정 전송하기", type="primary"):
            if plan_p_name != "---":
                m = st.session_state.m_states[plan_m_name]
                if m.get('p_name', '---') == '---':
                    m.update({'p_name': plan_p_name, 'target': plan_target, 'count': 0, 'is_running': False, 'last_time': time.time()})
                    clear_widget_state(plan_m_name)
                    st.success(f"✅ {plan_m_name} 기계가 비어있어 즉시 생산 제품으로 장착되었습니다!")
                else:
                    if 'schedule' not in m: m['schedule'] = []
                    m['schedule'].append({'p_name': plan_p_name, 'target': plan_target, 'date': plan_date})
                    st.success(f"✅ {plan_m_name} 기계의 대기열에 일정이 추가되었습니다!")
                save_machine_data(st.session_state.m_states)
                time.sleep(1)
                st.rerun()
            else:
                st.warning("유효한 제품을 선택해주세요.")
    ...
    def build_table_data(machines_list):
        ...
        for m_name in machines_list:
            ...
            max_cols = max(max_cols, len(tasks))
            raw_table_data.append({"기계명": m_name, "tasks": tasks})

        display_cols = max_cols + 3
        df_list = []
        for row in raw_table_data:
            d = {"기계명": row["기계명"]}
            for i in range(display_cols):
                d[f"{i+1}순서"] = row["tasks"][i] if i < len(row["tasks"]) else ""
            df_list.append(d)
            ...
            for i in range(d_cols):
                ...
                if cell_val != "" and not any(marker in cell_val for marker in ["✅", "🔄", "⏸️", "⏳"]):
                    ...
                    for p, info in st.session_state.master_data.items():
                        if info.get('p_code') == typed_str:
                            matched_p_name = p
                            break
                    
                    if not matched_p_name and typed_str in st.session_state.master_data:
                        matched_p_name = typed_str
    ...
    if changes_made:
        save_machine_data(st.session_state.m_states)
        st.success("✅ 제품 코드를 인식하여 일정이 자동 추가되었습니다!")
        time.sleep(1)
        st.rerun()
        
    st.write("---")
    st.markdown("<div style='font-size:13px; color:#86868b;'>💡 수정이나 삭제를 원하실 경우, 각 기계의 [⚙️ Details] 탭에서 대기열을 관리해 주세요.</div>", unsafe_allow_html=True)
...
with t_admin:
    ...
    if not st.session_state.is_admin:
        st.info("🔒 기계 및 제품 데이터를 수정하려면 관리자 권한이 필요합니다.")
        ...
        with col_login1:
            ...
            if st.button("로그인"):
                if pwd == ADMIN_PASSWORD:
                    ...
                    st.success("✅ 로그인 성공! 관리자 모드가 활성화되었습니다.")
                    time.sleep(0.5)
                    st.rerun()
                else:
        ...
        st.write("---")
        st.markdown("#### 📋 현재 등록된 제품 목록 (읽기 전용)")
        master_list = []
        for p_name, info in st.session_state.master_data.items():
            master_list.append({
                "제품코드": info.get("p_code", ""),
                "제품명": p_name,
                "컬러": info.get("color_text", ""),
                "무게(g)": info.get("weight", 0.0),
                "사이클속도(초)": info.get("cycle_time", 10)
            })
        st.dataframe(pd.DataFrame(master_list), use_container_width=True)

    else:
        if st.button("🔓 로그아웃 (일반 모드로 돌아가기)"):
            st.session_state.is_admin = False
            st.rerun()
        ...
        with col_admin1:
            ...
            with st.expander("➕ 제품 등록 및 수정", expanded=True):
                ...
                if st.button("제품 적용하기"):
                    if a_p_name.strip():
                        st.session_state.master_data[a_p_name] = {
                            "p_code": a_p_code, "color_text": a_p_color, 
                            "weight": a_p_weight, "cycle_time": a_p_cycle
                        }
                        save_master_data(st.session_state.master_data); st.success("제품 저장 완료!"); time.sleep(1); st.rerun()
                    else:
                        st.warning("제품명은 반드시 입력해야 합니다.")
                        
            st.write("---")
            master_list = []
            for p_name, info in st.session_state.master_data.items():
                master_list.append({
                    "제품코드": info.get("p_code", ""), "제품명": p_name,
                    "컬러": info.get("color_text", ""), "무게(g)": info.get("weight", 0.0),
                    "사이클(초)": info.get("cycle_time", 10)
                })
            st.dataframe(pd.DataFrame(master_list), use_container_width=True)
            ...
            if st.button("선택 제품 영구 삭제"):
                if del_p_name != "---":
                    del st.session_state.master_data[del_p_name]
                    save_master_data(st.session_state.master_data); st.success("삭제 완료!"); time.sleep(1); st.rerun()
            ...
            with st.expander("➕ 새 기계 라인 추가", expanded=True):
                ...
                if st.button("기계 추가하기"):
                    if new_m_name.strip() and new_m_name not in st.session_state.m_states:
                        st.session_state.m_states[new_m_name] = {'count': 0, 'last_time': time.time(), 'is_running': False, 'p_name': "---", 'target': 1000, 'schedule': [], 'history': [], 'floor': get_floor_from_machine(new_m_name), 'memo': ""}
                        save_machine_data(st.session_state.m_states); st.success("기계 생성 완료!"); time.sleep(1); st.rerun()
            st.write("---")
            del_m_name = st.selectbox("철거/삭제할 기계 선택", list(st.session_state.m_states.keys()))
            if st.button("선택 기계 영구 삭제"):
                del st.session_state.m_states[del_m_name]
                save_machine_data(st.session_state.m_states); st.success("기계 철거 완료!"); time.sleep(1); st.rerun()
