...
with t1:
    ...
    # E-IN 851AD 와 같은 이름 안에서 '851' 숫자를 찾아내는 마법의 함수!
    def find_real_name(target_num_str):
        for m_name in f1_machines:
            # 기계 이름에서 숫자만 쏙쏙 뽑아냅니다.
            extracted_nums = re.findall(r'\d+', m_name) 
            if target_num_str in extracted_nums:
                return m_name
        return None

    for left_num, right_num in f1_layout_pairs:
        # 도면의 숫자(예: 851)를 던져주고, 실제 등록된 기계 이름(예: E-IN 851AD)을 찾아옵니다.
