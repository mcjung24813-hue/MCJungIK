...
if not st.session_state.m_states:
    # 최초 실행 시 기본 생성용 
    default_machines = ["851", "852", "651", "652"]
...
# --- 💡 사용자가 짚어주신 1층 정확한 지도 배치 적용 ---
with t1:
    # 수정한 도면 번호 배열 (왼쪽 10대, 오른쪽 10대 완벽 대칭)
    f1_left_layout = [
        "851", "852", "651", "652", "551", "552", "553", "554", "655", "656"
    ]
    f1_right_layout = [
        "854", "853", "654", "653", "5510", "559", "558", "557", "556", "555"
    ]
    
    cols1 = st.columns(2)
    
    with cols1[0]:
        for m_name in f1_left_layout:
            if m_name in st.session_state.m_states:
                render_unified_machine_card(m_name)
                
    with cols1[1]:
        for m_name in f1_right_layout:
            if m_name in st.session_state.m_states:
                render_unified_machine_card(m_name)
                
    # 도면에 없는 번호(기타)의 기계가 있을 경우 밑으로 뺌
    layout_all = set(f1_left_layout + f1_right_layout)
    leftovers = [m for m in f1_machines if m not in layout_all]
