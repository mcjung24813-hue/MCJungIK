...
if not st.session_state.m_states:
    # 최초 실행 시 기본 생성용 
    default_machines = ["851", "854", "651", "654"]
...
# --- 💡 [수정됨] 1층 줄맞춤(Row-by-Row) 좌우 대칭 레이아웃 ---
with t1:
    # 요청하신 도면 쌍 (왼쪽, 오른쪽)
    f1_layout_pairs = [
        ("851", "854"),
    ...
    # 짝꿍끼리 무조건 같은 가로줄(Row)에 그려지도록 묶어버립니다.
    for left_m, right_m in f1_layout_pairs:
        layout_all_set.add(left_m)
        layout_all_set.add(right_m)
        
        c1, c2 = st.columns(2)
        with c1:
            if left_m in st.session_state.m_states:
                render_unified_machine_card(left_m)
        with c2:
            if right_m in st.session_state.m_states:
                render_unified_machine_card(right_m)
                
    # 도면에 없는 번호(기타)의 기계가 있을 경우 밑으로 뺌
    leftovers = [m for m in f1_machines if m not in layout_all_set]
    if leftovers:
        st.markdown("---")
        st.caption("※ 1층 도면 배치도에 포함되지 않은 기계들 (설정에서 이름을 확인하세요)")
    ...
    if not st.session_state.is_admin:
        ...
        with col_admin2:
            ...
            with st.expander("➕ 새 기계 라인 추가", expanded=True):
                # 사용자가 1층에 "851"이라는 기계를 원하면, 여기서 정확히 "851"로 기계를 추가해야 제자리에 들어갑니다!
                new_m_name = st.text_input("추가할 기계 이름 (예: 851)")
